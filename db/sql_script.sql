...
(
    Id          INT IDENTITY (1,1)
        CONSTRAINT PK_Month_Type PRIMARY KEY,
...
(
    ...
    Month_Type_Id INT NOT NULL
        CONSTRAINT FK_Month_Type FOREIGN KEY REFERENCES test.Month_Type (Id),
    Year_Id       INT NOT NULL
        CONSTRAINT FK_Year FOREIGN KEY REFERENCES test.Year (Id)
);
GO
...
(
    Month_Category_Id INT           NOT NULL IDENTITY (1,1)
        CONSTRAINT PK_Month_Category PRIMARY KEY,
    Category_Id       INT           NOT NULL
        CONSTRAINT FK_Category FOREIGN KEY REFERENCES test.Category (Category_Id),
    Month_Id          INT           NOT NULL
        CONSTRAINT FK_Month FOREIGN KEY REFERENCES test.Month (Month_Id),
...
exec sp_addextendedproperty 'MS_Description',
     N'info czy ta kategoria w miesiącu jest aktywna czy nie - np gdy w danym miesiący nie są konieczne żadne opłaty dla kategorii',
     'SCHEMA',
     'test',
     'TABLE',
     'Month_Category',
...
(
    Month_Category_Value_Id INT NOT NULL IDENTITY (1,1)
        CONSTRAINT PK_Month_Category_Value PRIMARY KEY,
    Month_Category_Id       INT NOT NULL
        CONSTRAINT FK_Month_Category FOREIGN KEY REFERENCES test.Month_Category (Month_Category_Id),
    Amount                  DECIMAL(10, 2),
    Date                    DATE,
    Name                    varchar(200),
    Valid                   BIT
